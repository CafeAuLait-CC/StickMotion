...
def recover_root_rot_pos(data): #[b,T,D]
    rot_vel = data[..., 0] # The rotational speed of the y-axis
    r_rot_ang = torch.zeros_like(rot_vel).to(data.device)
    '''Get Y-axis rotation from rotation velocity'''
    r_rot_ang[..., 1:] = rot_vel[..., :-1] 
    r_rot_ang = torch.cumsum(r_rot_ang, dim=-1) # Accumulate to obtain the rotation angle.

    r_rot_quat = torch.zeros(data.shape[:-1] + (4,)).to(data.device).to(data.dtype) # [B, T-1, 4] quaternion
    r_rot_quat[..., 0] = torch.cos(r_rot_ang) # w
    r_rot_quat[..., 2] = torch.sin(r_rot_ang) # y

    r_pos = torch.zeros(data.shape[:-1] + (3,)).to(data.device).to(data.dtype) # [B, T, 3]
    r_pos[..., 1:, [0, 2]] = data[..., :-1, 1:3] # The x and z coordinates of the root
...
def plot_3d_motion_kit(save_path, kinematic_tree, joints, title, figsize=(10, 10), fps=120, radius=4):
    matplotlib.use('Agg')
    ...
    def init():
        ax.set_xlim3d([-radius / 0.125, radius / 0.125])
        ax.set_ylim3d([0, radius / 0.0625])
        ax.set_zlim3d([0, radius / 0.0625])
        fig.suptitle(title, fontsize=20)
        ax.grid(b=False)
    ...
    ani = FuncAnimation(fig, update, frames=frame_number, interval=500 / fps, repeat=False)
    ani.save(save_path, fps=fps)
    plt.close()
