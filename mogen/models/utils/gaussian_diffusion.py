...
class ScheduleSampler(ABC):
    ...
    def sample(self, batch_size, device):
        ...
        indices = th.multinomial(p, batch_size, replacement=True, generator=None, out=None).long()
        weights = 1 / (len(p) * p[indices]).float()
        return indices, weights
    
    def to(self, device):
        """
        Move the sampler to a new device.
        """
        self._weights = th.tensor(self._weights, device=device)
    ...
    def to(self, device):
        for k, v in self.__dict__.items():
            if isinstance(v, np.ndarray):
                self.__dict__[k] = th.tensor(v).to(device)
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            model_variance, model_log_variance = {
                ...
                ModelVarType.FIXED_LARGE: (
                    ...
                    th.cat((self.posterior_variance[1].unsqueeze(0), self.betas[1:])),
                    # np.log(np.append(self.posterior_variance[1], self.betas[1:])),
                    th.log(th.cat((self.posterior_variance[1].unsqueeze(0), self.betas[1:]))),
                ),
