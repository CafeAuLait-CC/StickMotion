...
class RetrievalDatabase(nn.Module):
    def __init__(
        ...
        self.text_encoder = nn.TransformerEncoder(
            TransEncoderLayer, num_layers=num_layers
        )
        self.results = {}
    ...
    def encode_text(self, text, device):
        with torch.no_grad():
            text = clip.tokenize(text, truncate=True).to(device)
            x = self.clip.token_embedding(text).type(
                self.clip.dtype
    ...
    def retrieve(self, caption, length, clip_model, device, idx=None):
        if self.training and self.train_indexes is not None and idx is not None:
            ...
            rel_length = torch.abs(rel_length - length) / torch.clamp(
                rel_length, min=length
            )
    ...
    def forward(self, captions, lengths, clip_model, device, idx=None):
        ...
        for b_ix in range(B):
            ...
            if idx is None:
                batch_indexes = self.retrieve(
                    captions[b_ix], length, clip_model, device
                )
            else:
                batch_indexes = self.retrieve(
                    captions[b_ix], length, clip_model, device, idx[b_ix]
                )
        ...
        re_motion = self.motion_proj(all_motions) + self.motion_pos_embedding.unsqueeze(
            0
        )
        ...
        re_text = (
            re_text.permute(1, 0, 2).view(B, self.num_retrieval, T, -1).contiguous()
        )
...
class MultiJointEncoder(nn.Module):
    def __init__(self, d_model=512, joint_num=21):
        super().__init__()
        # self.stick_encoder = StickmanEncoder(stick_encoder)
        self.joint_num = joint_num
        self.proj_in = nn.Linear(joint_num * 2, d_model)
...
class ReMoDiffuseTransformer(DiffusionTransformer):
    def __init__(
        ...
        **kwargs,
        ...
        self.multijoint_encoder = MultiJointEncoder(**multijoint_encoder)
        ...
        self.both_p = text_p * joint_p
        self.text_only_p = (1 - joint_p) * text_p
        self.joint_only_p = (1 - text_p) * joint_p
        self.none_p = (1 - text_p) * (1 - joint_p)
    ...
    def get_precompute_condition(
        ...
        **kwargs,
        ...
        if joint_encoder is None:
            joint_encoder = self.multijoint_encoder(front_joints)
        output["joint_encoder"] = joint_encoder
        return output
    ...
    def forward_train(
        self, h=None, src_mask=None, emb=None, xf_out=None, joint_encoder=None, **kwargs
    ):
        # def forward_train(self, h=None, src_mask=None, emb=None, xf_out=None, stick_encoder=None, **kwargs):
        B, T = h.shape[0], h.shape[1]
        p_batch = [
            int(self.text_only_p * B),
            int(self.both_p * B),
            int(self.joint_only_p * B),
        ]
        # p_batch = [int(self.text_only_p * B), int(self.both_p * B), int(self.stick_only_p * B)]
        joint_mask = (
            torch.rand((B, self.index_num, 1), device=h.device) < self.index_p
        ).int()
        ...
        return output, index, p_batch, joint_mask
    ...
    def forward_test(
        ...
        **kwargs,
        ...
        joint_encoder = joint_encoder.repeat(4, 1, 1)
        ...
        joint_mask = torch.ones((4 * B, self.index_num, 1), device=h.device)
