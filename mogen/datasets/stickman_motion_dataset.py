...
class Stickmant2mDataset(BaseMotionDataset):
    ...
    def __init__(
        ...
        if token_dir is not None:
            self.token_dir = os.path.join(
                data_prefix, "datasets", dataset_name, token_dir
            )
        else:
            self.token_dir = None
        if clip_feat_dir is not None:
            self.clip_feat_dir = os.path.join(
                data_prefix, "datasets", dataset_name, clip_feat_dir
            )
        else:
    ...
    def prepare_data(self, idx: int):
        ...
        ref_centered = ref - center
        _, _, vh = LA.svd(ref_centered, full_matrices=False)
        proj_mat = vh[:2]

        front_joints = []
        for j in joints:
            proj = (j - center) @ proj_mat.T
            front_joints.append(proj)
        results["front_joints"] = np.array(front_joints, dtype=np.float32)
