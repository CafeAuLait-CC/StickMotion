...
if __name__ != "__main__":
    ...
    pytest.skip(
        "lg_test is an executable script, not a test module", allow_module_level=True
    )
...
def parse_args():
    ...
    parser.add_argument(
        "gpu", help="-1: 0,1,2,3; -2: 4,5,6,7; else: set to gpu directly"
    )
...
def main():
    ...
    if Fabric().global_rank == 0:
        ...
        pid_seed = os.getppid()
    model = LgModel(
        cfg, dataset, unit=hashlib.md5(str(pid_seed).encode()).hexdigest()[:8]
    )
